class W3Color {
	constructor(enterType = "hex", values = []){
    const types = [{
      type:"hex",
      valslength:1,
      build: () => values[0]
      ...
      type:"rgb",
      valslength:3,
      build: () => "rgb(" + values[0] + ", " + values[1] + ", " + values[2] + ")"
    }, {
      type:"hsl",
      valslength:3,
      build: () => "hsl(" + values[0] + ", " + values[1] + "%, " + values[2] + "%)"
    }, {
      type:"hwb",
      valslength:3,
      build: () => "hwb(" + values[0] + ", " + values[1] + "%, " + values[2] + "%)"
    }, {
      type:"cmyk",
      valslength:4,
      build: () => "cmyk(" + values[0] + "%, " + values[1] + "%, " + values[2] + "%, " + values[3] + "%)"
    }, {
      type:"hsv",
      valslength:3,
      build: () => {
        const point = (200 - values[1]) * values[2] / 100;
        const h = values[0],
          s = (point === 0 || point === 200) ? 0 : values[1] * values[2] / 100 / ((point <= 100) ? point : 200 - point) * 100,
          l = point * 5 / 10;
        return "hsl(" + h + ", " + s + "%, " + l + "%)";
      }
    }];
    
    const typesIndex = types.findIndex(item => item.type == enterType && item.valslength == values.length);
    if(typesIndex < 0)
      return;
    const color = types[typesIndex].build();
  ...
  toHwb(decimal = false){
    const h = this.hue,
      w = decimal ? this.whiteness : Math.round(this.whiteness * 100),
      b = decimal ? this.blackness : Math.round(this.blackness * 100);
    return { h, w, b };
  }
	toHwbString(){
  ...
  toHwba(){
	...
	toHwbaString(){
    const { h, w, b, a } = this.toHwba();
		return "hwba(" + h + ", " + w + "%, " + b + "%, " + a + ")";
	}
  toHsl(decimal = false){
    const h = this.hue,
      s = decimal ? this.sat : Math.round(this.sat * 100),
      l = decimal ? this.lightness : Math.round(this.lightness * 100);
    return { h, s, l };
  }
	toHslString(){
  ...
  toHsv(){
    const hsl = this.toHsl();
    const midPoint = hsl.s * (hsl.l < 50 ? hsl.l : 100 - hsl.l) / 100;
    const s = midPoint === 0 ? 0 : 2 * midPoint / (hsl.l + midPoint) * 100;
    const v = hsl.l + midPoint;
    return { h: hsl.h, s, v };
  }
  toCmyk(decimal = false){
    const c = decimal ? this.cyan : Math.round(this.cyan * 100),
      m = decimal ? this.magenta : Math.round(this.magenta * 100),
      y = decimal ? this.yellow : Math.round(this.yellow * 100),
      k = decimal ? this.black : Math.round(this.black * 100);
    return { c, m, y, k };
  }
	toCmykString(){
	...
	toCmykStringDecimal(){
    const { c, m, y, k } = this.toCmyk(true);
		return "cmyk(" + c + ", " + m + ", " + y + ", " + k + ")";
	}
	toNcolString(){
