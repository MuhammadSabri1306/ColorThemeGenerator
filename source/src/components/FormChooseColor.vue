...
onMounted(() => {
	if(getActiveTabs == "customColor" && value.length < 1)
		...
		twDataList.value.searchField.focus();
});
</script>
<template>
	<FixedModal @close="$emit('cancel')">
		...
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-0">
			<div class="lg:border-r lg:border-r-gray-200">
				<DataList ref="twDataList" v-if="getActiveTabs() == 'twPalette'" :model="tailwindPaletteList" @cancel="$emit('cancel')" @change="chooseTailwindColor" />
			...
			<div class="flex flex-col justify-stretch lg:pl-8">
				...
				<div class="flex justify-end pl-8 pr-8 mt-auto lg:pr-16">
					<button ref="btnSubmit" @click="submit" :disabled="value.length < 1" type="button" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none">Submit</button>
				</div>
			</div>
		</div>
	</FixedModal>
