...
<template>
	<FixedModal @close="$emit('cancel')" class="modal-w-auto">
		...
		<div class="flex justify-end px-8">
			<button type="button" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 enabled:hover:bg-indigo-700 focus:outline-none" :disabled="key.length < 1" @click="$emit('submit', key)">Choose Color</button>
		</div>
	</FixedModal>
</template>
