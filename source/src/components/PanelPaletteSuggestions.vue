...
<template>
	<div>
		<div v-if="suggestions.length > 0" class="grid grid-cols-1 md:grid-cols-2 gap-16">
			<a v-for="item in suggestions" :href="createUrl(item.id)" class="inline-flex flex-col items-stretch group bg-white hover:bg-indigo-700 border border-gray-300 hover:border-indigo-700 shadow hover:shadow-lg rounded-xl overflow-hidden transition-all duration-200 ease-in-out">
